...
<section class="result-section">
    <div class="container">
        <!-- 我的寻宠发布（核心修复） -->
        <h3 class="mb-4"><i class="bi bi-person me-2"></i>我的寻宠发布</h3>
        <div class="row g-4 mb-5">
            <c:choose>
                ...
                <c:when test="${empty sessionScope.userId}">
                    <div class="col-12">
                        <div class="empty-state">
                            ...
                            <p class="text-muted">登录后即可查看您发布的寻宠信息</p>
                            <a href="${pageContext.request.contextPath}/login_choice.jsp" class="btn btn-search mt-3">
                ...
                <%-- 2. 用户已登录但无发布记录（修复判断条件） --%>
                <c:when test="${empty myPetSearchList or myPetSearchList.size() == 0}">
                    <div class="col-12">
                        <div class="empty-state">
                            <div class="empty-state-icon">
                                <i class="bi bi-inbox"></i>
                            </div>
                            <h4 class="text-muted">暂无发布记录</h4>
                            <p class="text-muted">您还未发布任何寻宠信息，立即发布吧！</p>
                            <button class="btn btn-search mt-3" onclick="document.getElementById('searchForm').scrollIntoView({behavior:'smooth'})">
                ...
                <%-- 3. 用户已登录且有发布记录（修复数据渲染） --%>
                <c:otherwise>
                    <c:forEach items="${myPetSearchList}" var="search">
                        <div class="col-md-6 col-lg-4">
                            <div class="card pet-card" style="border: 2px solid var(--primary-color);">
                                <!-- 图片显示 -->
                                <c:choose>
                                ...
                                <div class="card-body">
                                    <!-- 我的发布标记 -->
                                    <span class="badge bg-success mb-2">我的发布</span>
                                    ...
                                    <div class="mt-3">
                                        <button class="btn btn-sm btn-outline-primary me-2"
                                                onclick="location.href='${pageContext.request.contextPath}/pet/search?action=edit&id=${search.id}'">
                                            <i class="bi bi-pencil me-1"></i>编辑
                                        </button>
                                        <button class="btn btn-sm btn-outline-danger"
                                                onclick="if(confirm('确定删除这条寻宠信息吗？')) location.href='${pageContext.request.contextPath}/pet/search?action=delete&id=${search.id}'">
