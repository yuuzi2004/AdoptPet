...
<script>
    ...
    function confirmDeleteApplication(applicationId, petName) {
        ...
        fetch('${pageContext.request.contextPath}/adoption/delete', {
            ...
            var ok = !!data.success;
            var msg = data.msg || (ok ? '删除成功' : '删除失败，请稍后重试');
            showResultModal(ok, msg);
        }).catch(function () {
            showResultModal(false, '请求失败，请检查网络后重试');
        });
    }
    ...
    // 显示薄巧色结果弹窗，并在成功时关闭后刷新列表
    function showResultModal(success, message) {
        var modalEl = document.getElementById('deleteResultModal');
        if (!modalEl) return;

        var iconEl = document.getElementById('deleteResultIcon');
        var textEl = document.getElementById('deleteResultText');

        textEl.textContent = message;
        if (success) {
            iconEl.className = 'bi bi-check-circle-fill text-success pastel-modal-icon';
        } else {
            iconEl.className = 'bi bi-exclamation-circle-fill text-danger pastel-modal-icon';
        }
    ...
    // 页面加载后根据 URL 参数显示一次弹窗（兼容从其它操作跳转过来的提示）
    (function () {
        var successMsg = '${param.success != null ? param.success : ""}';
        var errorMsg = '${param.error != null ? param.error : ""}';
        var message = successMsg || errorMsg;
        if (!message) {
            return;
        }
        showResultModal(!!successMsg, message);
    })();
</script>
</body>
</html>