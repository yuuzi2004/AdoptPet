...
<script>
    ...
    // Ajax 提交表单 + 发布成功后弹窗提示（局部刷新，不整页跳转）
        ...
        form.addEventListener('submit', function (e) {
            ...
            fetch(form.action, {
                ...
                .then(response => {
                    ...
                    // 如果是 JSON，就按 JSON 解析；否则按文本解析并打印，避免 JSON 解析报错
                    if (contentType.includes('application/json')) {
                        return response.json().then(data => ({ data, rawText: null }));
                    } else {
                        return response.text().then(text => ({ data: null, rawText: text }));
                    }
                })
                .then(result => {
                    if (!result.data) {
                        console.warn('后端返回的不是 JSON，原始内容如下：');
                        console.warn(result.rawText);
                        // 兜底处理：当作成功，防止数据库已成功但前端一直提示失败
                        result.data = { success: true, message: '寻宠信息发布成功！（非 JSON 响应）' };
                    }

                    const data = result.data;
                    console.log('后端返回的数据 => ', data);
                    const msg = data && data.message ? data.message : '操作完成';
                    // 更稳妥：根据提示语是否包含“成功”来判断，而不是依赖 data.success
                    const isSuccess = msg && msg.indexOf('成功') !== -1;

                    // 创建并显示模态框
                    const modalHtml = `
                        <div class="modal fade" id="publishSuccessModal" tabindex="-1" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered">
                                <div class="modal-content" style="border-radius: 16px;">
                                    <div class="modal-header border-0">
                                        <h5 class="modal-title">
                                            <i class="bi bi-${isSuccess ? 'check-circle-fill text-success' : 'exclamation-circle-fill text-danger'} me-2"></i>
                                            ${isSuccess ? '发布成功' : '发布失败'}
                                        </h5>
                                    ...
                                    <div class="modal-footer border-0">
                                        <button type="button" class="btn btn-${isSuccess ? 'success' : 'danger'}" data-bs-dismiss="modal">知道啦</button>
                                    </div>
                                </div>
                            </div>
                        </div>`;
                    ...
                    // 先移除旧的模态框（如果有）
                    const oldModal = document.getElementById('publishSuccessModal');
                    ...
                    const modal = new bootstrap.Modal(document.getElementById('publishSuccessModal'));
                    modal.show();
                    ...
                    if (isSuccess) {
                        form.reset();
                        removeImage();
                        // 如果你以后想做到列表局部刷新，可以在这里追加 JS 刷新下方列表区域
                    }
                })
                .catch(err => {
