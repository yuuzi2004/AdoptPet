...
public class PetAddServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) 
        ...
        try {
            ...
            try {
                success = petService.addPet(pet);
            } catch (Exception e) {
                e.printStackTrace();
                errorMsg = "保存宠物信息时发生错误：" + e.getMessage();
                req.setAttribute("error", errorMsg);
                req.getRequestDispatcher("/add.jsp").forward(req, resp);
                return;
            }
            ...
            if (success) {
                ...
                try {
                    ...
                    e.printStackTrace();
                    // 如果重定向失败，至少返回成功信息到表单页面
                    req.setAttribute("success", "提交成功！宠物信息已添加到领养列表。");
                    req.getRequestDispatcher("/add.jsp").forward(req, resp);
                }
            } else {
                // 新增失败，返回错误信息
                errorMsg = "提交失败，请稍后重试！可能是网络问题或服务器错误。";
                req.setAttribute("error", errorMsg);
                req.getRequestDispatcher("/add.jsp").forward(req, resp);
            }
        } catch (Exception e) {
            // 捕获所有未预期的异常，确保不会返回空白页
            e.printStackTrace();
