...
public class PetDaoImpl implements PetDao {
    ...
    public List<Pet> findPetsByUserId(Integer userId) {
        ...
        try {
            conn = JdbcUtils.getConnection();
            String sql = "SELECT id, name, type, age, gender, description, image_path, user_id FROM pet WHERE user_id = ? ORDER BY create_time DESC";
            pstmt = conn.prepareStatement(sql);
    ...
    public boolean updatePet(Pet pet) {
        ...
        try {
            conn = JdbcUtils.getConnection();
            String sql = "UPDATE pet SET name=?, type=?, age=?, gender=?, description=?, image_path=? WHERE id=? AND user_id=?";
            pstmt = conn.prepareStatement(sql);
            pstmt.setString(1, pet.getName());
            pstmt.setString(2, pet.getType());
            pstmt.setInt(3, pet.getAge());
    ...
    @Override
    public boolean deletePet(Integer petId, Integer userId) {
        Connection conn = null;
        PreparedStatement pstmt = null;
        try {
            conn = JdbcUtils.getConnection();
    ...
    @Override
    public Pet findPetByIdAndUserId(Integer id, Integer userId) {
        Connection conn = null;
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        try {
            conn = JdbcUtils.getConnection();
            String sql = "SELECT id, name, type, age, gender, description, image_path, user_id FROM pet WHERE id = ? AND user_id = ?";
            pstmt = conn.prepareStatement(sql);
            pstmt.setInt(1, id);
            pstmt.setInt(2, userId);
            rs = pstmt.executeQuery();
     ...
     * 管理员删除宠物（不需要验证userId）
     */
    public boolean deletePetByAdmin(Integer petId) {
        Connection conn = null;
        PreparedStatement pstmt = null;
        try {
            conn = JdbcUtils.getConnection();
